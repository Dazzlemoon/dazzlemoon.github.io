...
Hu 等 2025 用 **DP-QEq** 把 **QEq 长程静电** 与 **Deep Potential 短程** 拆开训练，在 **Li / [EC+LiPF$\_6$] / Li** 双电极模型上实现 **循环恒电势 MD**，首次在 ML 加速框架下 **直接看到** SEI 无机相中 **Li 聚集 → 不均匀沉积 → 锂枝晶成核** 的原子过程。**PQEq 基础势**（Gao 2025）与 **ConstP** 机制互补、可原则性结合为「短程 ML + PQEq + D3 + 电极 $\phi\_i$」。
